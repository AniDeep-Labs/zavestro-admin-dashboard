...
import type { Banner, BannerPayload, BannerLayout, BannerTextPosition, BannerTextColor, BannerImageFit, BannerMode, BannerCtaStyle } from '../../api/adminApi';
...
const LAYOUTS: LayoutMeta[] = [
  ...
  { id: 'triptych',    label: 'Triple panel',    on: ['web'] },
...
const layoutsFor = (dev: 'mobile' | 'web') => LAYOUTS.filter(l => l.on.includes(dev));
...
function BannerHero({ data, frame }: { data: HeroData; frame: 'mobile' | 'web' | 'fill' }) {
  ...
  switch (layout) {
    ...
    // ── Triple-panel: one hero image + two product panels (refs: editorial grids) ──
    case 'triptych': {
      const panels = thumbUrls.slice(0, 2);
      return (
        <div className={heroCls} style={heroAR}>
          <div className={b.heroRow}>
            <div className={b.tpMain} style={imageUrl ? undefined : { background: gradient }}>
              {Img}<div className={b.tpScrim} />
              <div className={b.tpText}>
                {tag && <span className={b.heroTag}>{tag}</span>}
                <span className={b.heroTitle}>{(title || 'Headline').replace(/\\n/g, '\n')}</span>
                <span className={b.heroCta}>{ctaText || 'Shop Now'}</span>
              </div>
            </div>
            <div className={b.tpSide}>
              {[0, 1].map(i => (
                <div key={i} className={b.tpPanel} style={panels[i] ? undefined : { background: gradient }}>
                  {panels[i] ? <img src={panels[i]} alt="" /> : <span className={b.heroEmpty}>+ thumb</span>}
                </div>
              ))}
            </div>
          </div>
    ...
    // ── Premium split: image + light editorial panel, serif headline, rule, pill ──
    case 'split':
    case 'editorial':
      return (
        <div className={heroCls} style={heroAR}>
          <div className={b.heroRow}>
            <div className={b.edImg} style={imageUrl ? undefined : { background: gradient }}>
              {Img}<div className={b.edFade} />
            </div>
            <div className={b.edPanel}>
              {tag && <span className={b.edTag}>{tag}</span>}
              <span className={b.edTitle}>{(title || 'Headline').replace(/\\n/g, '\n')}</span>
              <span className={b.edLine} />
              {subtitle && <span className={b.edSub}>{subtitle}</span>}
              <span className={b.edCta}>{ctaText || 'Shop Now'} →</span>
            </div>
          </div>
        </div>
      );
    // ── Text + product: bold copy on colour, product image floated right ──
    case 'text_cutout':
      return (
        <div className={heroCls} style={{ background: gradient, ...heroAR }}>
          <div className={b.heroRow}>
            <div className={`${b.tiText} ${colCls}`}>
              {tag && <span className={b.heroTag}>{tag}</span>}
              <span className={b.heroTitle}>{(title || 'Headline').replace(/\\n/g, '\n')}</span>
              {subtitle && <span className={b.heroSub}>{subtitle}</span>}
              <span className={b.heroCta}>{ctaText || 'Shop Now'}</span>
...
function BannerForm({
  ...
  const initType: 'collection' | 'category' | 'url' =
    initLink.startsWith('/collections/') ? 'collection' : initLink.startsWith('/categories') ? 'category' : 'url';
  const [linkType, setLinkType] = React.useState(initType);
  const [linkValue, setLinkValue] = React.useState(
    initType === 'collection' ? initLink.replace('/collections/', '')
      : initType === 'category' ? initLink.replace('/categories/', '').replace('/categories', '')
        : initLink,
  );
  const ctaLink = linkType === 'collection' ? `/collections/${linkValue.trim()}`
  ...
  const canSave = !!title.trim() && (!!imageMobile || !!imageWeb);
  ...
  const deviceHero = (frame: 'mobile' | 'web') => {
    ...
    return <BannerHero frame={frame} data={{ layout: lay, aspectRatio: ratio, title, subtitle, tag, ctaText, imageUrl: url, focalX: ux, focalY: uy, imageFit: 'cover', imageZoom: 100, bgColor1, bgColor2, textPosition, textColor, overlay, badgeText: badgeText || '50% OFF', logoUrl, showAd, thumbUrls, pills, gradientAngle, gradientSolid, ctaStyle }} />;
  };

  return (
    <form onSubmit={submit}>
      <div className={b.studio}>
        <div className={b.studioPanes}>
          <div className={b.formPane}>
            ...
            <div className={b.section}>
              ...
              <span className={b.hint}>{mode === 'upload' ? 'Upload a finished, designed banner (text baked into the artwork) — the premium path.' : 'Build it from text + image + a layout, in the CMS.'}</span>
            </div>

            {/* Image + focal (both modes) */}
            <div className={b.section}>
              ...
              <div className={b.fieldRowS}>
                <label className={b.label}>{dev === 'mobile' ? 'Mobile' : 'Web'} banner size — {aspect < 1 ? `tall ${aspect.toFixed(2)}:1` : `${aspect.toFixed(2)}:1`}</label>
                <input type="range"
                ...
                <span className={b.hint}>Drag to set how tall/short this device's banner is.</span>
              </div>
            </div>
            ...
            {mode === 'compose' && (<>
              ...
              <div className={b.section}>
                ...
                <div className={b.fieldRowS}><label className={b.label}>Headline *</label>
                  <textarea value={title} onChange={e => setTitle(e.target.value)} rows={2} className={b.input} placeholder={'Made to\nMeasure.'} /></div>
                <div className={b.fieldRowS}><label className={b.label}>Subtitle</label>
                  <input value={subtitle} onChange={e => setSubtitle(e.target.value)} className={b.input} /></div>
                ...
                {layout === 'story' && (
                  <div className={b.fieldRowS}>
                    ...
                    {logoKey ? (
                      <div className={b.imgRow}>
                        {logoUrl && <img src={logoUrl} alt="logo" className={b.logoThumb} />}
                        <span className={b.imgKey}>{logoKey.split('/').pop()}</span>
                        <button type="button" onClick={() => setLogoKey('')} className={b.imgRemove}>Remove</button>
                      </div>
                    ) : (
                      <button type="button" onClick={() => logoRef.current?.click()} className={`${b.input} ${b.chooseBtn}`}>＋ Upload logo (transparent PNG)</button>
                    )}
                    <span className={b.hint}>Shown in the white chip. Falls back to the tag text if empty.</span>
                  </div>
                )}
                {/* Shoppable extras (curated + triptych layouts) */}
                {(layout === 'curated' || layout === 'triptych') && (<>
                  <div className={b.fieldRowS}>
                    ...
                    <div className={b.thumbStrip}>
                      {thumbUrls.map((u, i) => (
                        <div key={i} className={b.thumbStripItem}>
                          <img src={u} alt="" />
                          <button type="button" onClick={() => setThumbKeys(prev => prev.filter((_, j) => j !== i))}>×</button>
                        </div>
                      ))}
                  ...
                  {layout === 'curated' && (
                    <div className={b.fieldRowS}>
                      ...
                      {pills.map((p, i) => (
                        <div key={i} className={b.pillRow}>
                          <input value={p} onChange={e => setPills(prev => prev.map((x, j) => j === i ? e.target.value : x))} className={b.input} placeholder="e.g. Grooming Set" />
                          <button type="button" onClick={() => setPills(prev => prev.filter((_, j) => j !== i))} className={b.imgRemove}>×</button>
                        </div>
                      ))}
              ...
              <div className={b.section}>
                ...
                <div className={b.grid2}>
                  <div className={b.fieldRowS}><label className={b.label}>{layout === 'split' ? 'Block colour' : 'Gradient 1'}</label>
                  ...
                  <div className={b.fieldRowS}><label className={b.label}>Gradient 2</label>
                ...
                <div className={b.fieldRowS}>
                  <label className={b.label}>Background fill</label>
                  <div className={b.segmented}>
                ...
                {!gradientSolid && (
                  <div className={b.fieldRowS}><label className={b.label}>Gradient angle — {gradientAngle}°</label>
                ...
                <div className={b.fieldRowS}>
                  <label className={b.label}>CTA style</label>
                  <div className={b.segmented}>
                    {(['auto', 'arrow', 'pill', 'none'] as BannerCtaStyle[]).map(c => (
                      <button type="button" key={c} className={`${b.seg} ${ctaStyle === c ? b.segActive : ''}`} onClick={() => setCtaStyle(c)}>{c}</button>
                    ))}
                  </div>
            ...
            <div className={b.section}>
              ...
              <div className={b.fieldRowS}><label className={b.label}>Internal name *</label>
                <input value={title} onChange={e => setTitle(e.target.value)} className={b.input} placeholder="e.g. Wedding Season Hero" />
          ...
          {/* Preview follows the active device tab */}
          <div className={b.previewPane}>
            ...
            {dev === 'mobile' ? (
              <div className={b.phone}><div className={b.phoneScreen}>
                ...
                <div className={b.phoneHero}>{deviceHero('mobile')}</div>
              ...
              <div className={b.browser}>
                <div className={b.browserBar}><div className={b.browserDots}><span className={`${b.bDot} ${b.bDotR}`} /><span className={`${b.bDot} ${b.bDotY}`} /><span className={`${b.bDot} ${b.bDotG}`} /></div><div className={b.browserAddr}>zavestro.com</div></div>
            ...
            <span className={b.hint}>
              {dev === 'web' && !imageWeb ? 'No web image yet — it falls back to the mobile image.' : ''}
            </span>
          </div>
        </div>
