...
const ROLE_CAP_SUMMARY: Record<string, string> = {
  design:
    "Designs + fabric match, sample review/verdict, fit analytics (read).",
  procurement:
    "Fabrics master, distribution to hubs, restock fulfilment, reports.",
  catalog_manager:
    "Listings + pricing + storefront CMS (their hub), restock requests, samples.",
  support:
    "Orders (CX: notes/hold/cancel/link-fit/re-measure), customers + credits (≤₹500), returns, reviews.",
  finance:
    "Refunds, COD confirmation, invoices, settlement/P&L (read). No floor or catalog writes.",
  pricing_manager:
    "Brand-wide promo codes + business analytics (read). Global. Does NOT set listing prices — that is the catalog manager (per hub).",
  super_admin:
    "Everything — hubs, staff, config, break-glass overrides, DPDP erasure. Oversight, not daily ops.",
};
...
export const AdminUsersManagePage: React.FC = () => {
  ...
  const handleApprovePending = async (user: AdminUser) => {
    ...
    if (!role) {
      showToast(
        "error",
        "Choose a role",
        "Pick the role this person should have before approving.",
      );
      return;
    }
  ...
  // [SHL-7-3] Assigning a role is the same act whether the account has never been
  // activated or is signed in with no capability at all, so it is one control rather
  // than a branch only one of those states can reach.
  const renderRoleAssign = (user: AdminUser, actionLabel: string) => (
    <div className={styles.cardActions}>
      ...
      >
        ...
        <option value="pricing_manager">
          Pricing &amp; Promotions (central)
        </option>
      ...
      {(() => {
        const role = approveRole[user.id];
        const hubScoped =
          role && role !== "super_admin" && role !== "pricing_manager";
        // [SHL-1-12] For a PER-HUB role there is no "Global (no hub)" — that option
        // promised oversight and delivered a globally scoped catalog_manager. Only
        // roles that may genuinely be unscoped still offer it.
        ...
        return hubScoped ? (
          <select
            ...
            onChange={(e) =>
              setApproveHub((h) => ({ ...h, [user.id]: e.target.value }))
            }
      ...
      <button
        ...
        disabled={
          !approveRole[user.id] ||
          approving === user.id ||
          (HUB_SCOPED_ROLES.includes(approveRole[user.id] ?? "") &&
            !approveHub[user.id])
        }
        onClick={() => handleApprovePending(user)}
      >
        {approving === user.id ? "Saving…" : actionLabel}
      </button>
    </div>
  );
  ...
  const handleCreate = async () => {
    ...
    if (HUB_SCOPED_ROLES.includes(newRole) && !newHubId) {
      showToast(
        "error",
        "Choose a hub",
        `${ROLE_LABELS[newRole] ?? newRole} manages one hub — pick which.`,
      );
      return;
    }
    setCreating(true);
    try {
      ...
      const hubId =
        newRole === "super_admin" || newRole === "pricing_manager"
          ? null
          : newHubId || null;
      const created = await catalogApi.createAdmin({
        name: newName.trim(),
        email: newEmail.trim(),
  ...
  const handleSetTempPw = async () => {
    ...
    try {
      ...
      showToast(
        "success",
        "Temp password set",
        `${showTempPw.email} must change password on next login`,
      );
      setShowTempPw(null);
      setTempPw("");
    } catch (err) {
  ...
  const handleSetHub = async (user: AdminUser, hubId: string) => {
    ...
    try {
      ...
      setUsers((prev) =>
        prev.map((u) =>
          u.id === user.id
            ? {
                ...u,
                hub_id: updated.hub_id ?? null,
                hub_name:
                  hubs.find((h) => h.id === updated.hub_id)?.name ?? null,
              }
            : u,
        ),
      );
      showToast(
        "success",
        hubId
          ? "Hub assigned — admin is now hub-scoped"
          : "Hub cleared — admin is global",
      );
    } catch (err) {
      showToast("error", "Failed", err instanceof Error ? err.message : "");
    } finally {
  ...
  // [SHL-7-3] Section by LIFECYCLE STATE, not by the `is_active` flag alone.
  //
  // The page sectioned on `is_active` but labelled by lifecycle, so the two disagreed in
  // both directions. On live data: 2 accounts holding real roles (one of them a
  // super_admin) sat under "Pending Activation" as if they were newcomers awaiting
  // approval — they are DEACTIVATED admins whose access was revoked. And 1 account with
  // no role at all — it can sign in and do literally nothing — was listed as an "Active
  // Admin". A roleless account is not the same thing as an activated one.
  const isRoleless = (u: { role?: string | null }) =>
    (u.role ?? "pending") === "pending";
  const deactivated = users.filter((u) => !u.is_active && !isRoleless(u));
  const awaitingActivation = users.filter((u) => !u.is_active && isRoleless(u));
  const activeNoRole = users.filter((u) => u.is_active && isRoleless(u));
  const active = users.filter((u) => u.is_active && !isRoleless(u));
  const pending = awaitingActivation;
  ...
  return (
    <div className={styles.page}>
      ...
      {pending.length > 0 && (
        <section className={styles.section}>
          <h2 className={styles.sectionTitle}>
            Awaiting activation — never signed in
          ...
          <div className={styles.cards}>
            {pending.map((user) => (
              ...
              >
                ...
                {renderRoleAssign(user, "Approve & activate")}
              </div>
            ))}
          </div>
        </section>
      ...
      {activeNoRole.length > 0 && (
        <section className={styles.section}>
          <h2 className={styles.sectionTitle}>
            Signed in, but no role assigned
            <span className={styles.badge}>{activeNoRole.length}</span>
          </h2>
          <p className={styles.sectionNote}>
            These accounts can log in and can do nothing — they hold no
            capability until a role is assigned. They used to be listed below as
            active admins.
          </p>
          <div className={styles.cards}>
            {activeNoRole.map((user) => (
              <div key={user.id} className={styles.card}>
                ...
                {renderRoleAssign(user, "Assign role")}
              </div>
            ))}
          </div>
        </section>
      ...
      {deactivated.length > 0 && (
        <section className={styles.section}>
          <h2 className={styles.sectionTitle}>
            ...
            <span className={styles.badge}>{deactivated.length}</span>
          </h2>
          <p className={styles.sectionNote}>
            These held a real role and were switched off. They are not newcomers
            awaiting approval, which is where this page used to list them.
          </p>
          <div className={styles.cards}>
              ...
              <div key={user.id} className={styles.card}>
                <div className={styles.cardInfo}>
                  ...
                  <div>
                    <div className={styles.userName}>{user.name}</div>
                    <div className={styles.userEmail}>{user.email}</div>
                    {/* The role is the point: deactivating a super_admin and deactivating
                        a support agent are not the same event. */}
                    <div className={styles.userMeta}>
                      was: {user.role ?? "—"}
                    </div>
                  </div>
              ...
              {loading ? (
                ...
                active.map((user) => (
                  <tr key={user.id}>
                    ...
                    <td>
                      <span
                        className={`${styles.rolePill} ${user.role === "super_admin" ? styles.roleSuperAdmin : styles.roleAdmin}`}
                      >
      ...
      {showTempPw && (
        <div
          ...
          onClick={() => setShowTempPw(null)}
        >
          <div
            className={styles.modal}
            {...tempPwDialog.dialogProps}
          ...
          >
            ...
            >
              Setting a temp password for <strong>{showTempPw.email}</strong>.
            ...
            <div className={styles.modalActions}>
              ...
              <button
                className={styles.activateBtn}
                disabled={settingTempPw}
                onClick={handleSetTempPw}
              >
            ...
            <div className={styles.fields}>
              ...
              {newRole !== "super_admin" && newRole !== "pricing_manager" && (
                <div className={styles.field}>
                  ...
                  >
                    <option value="">
                      {HUB_SCOPED_ROLES.includes(newRole)
                        ? "Choose a hub…"
                        : "All hubs (global)"}
                    </option>
                    {hubs.map((h) => (
                  ...
                  >
                    Assign a hub to restrict this admin to that hub's orders,
                    returns, invoices, staff & dashboard.
                  </span>
                </div>
              )}
            </div>
